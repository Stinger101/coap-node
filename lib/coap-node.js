...
CoapNode.prototype._updateNetInfo = function (callback) {
    ...
    network.get_active_interface(function(err, obj) {
        if (err) {
            // Look for address info from inside container
            var child_process = require('child_process');
            var Promise = require('promise');

            var IP =  new Promise(function (resolve, reject) {
                child_process.execFile('/bin/sh', ['-c', 'hostname -i'], function(err, stdout, stderr) {
                    if (err) reject(callback(err));
                    else resolve(stdout);
                });
            });

            var MAC = new Promise(function(resolve, reject) {
                child_process.execFile('/bin/sh', ['-c', 'ifconfig -a | grep HWaddr'], function(err, stdout, stderr) {
                    if (err) reject(callback(err));
                    else resolve(stdout);
                });
            });

            var GW = new Promise(function(resolve, reject) {
            ...
            Promise.all([IP, MAC, GW]).then(values => {
                self.ip = values[0].trim().toString();
                self.mac = values[1].split(" ").splice(-3, 1).toString();
                self.so.set('connMonitor', 0, 'ip', self.ip);
                self.so.set('connMonitor', 0, 'routeIp', values[2] || 'unknown');
                callback(null, { ip: self.ip, mac: self.mac, routeIp: values[2] });
            });
...
CoapNode.prototype.multicast = function (path, method, value, callback) {
  var self = this,
      reqObj = {
          host: '224.0.1.187',
          port: '5683',
          ...
          multicast: true,
          method: method,
          payload: cutils.encodeTlv(path, value),
      ...
      if (this._registered === true) {
          this.request(reqObj, agent, function (err, rsp) {
              if (err) {
                  ...
                  msg = { status: rsp.code};
                  if (rsp.code === RSP.changed || rsp.code === RSP.ok) {
                      setListenerStart(msg);
                  } else {
                      invokeCbNextTick(null, msg, callback);
                  }
...
function startMultiListener(cn, callback) {
    ...
    try {
        server_multi.listen('5683', '224.0.1.187', function (err) {
            if (err) {
                if (_.isFunction(callback))
                    callback(err);
                else
